...
var CVSS = function (id, options) {
    ...
    for (g in this.bg) {
        ...
        for (s in this.bm[g]) {
            ...
            if (f.querySelectorAll('dl').length % 4 === 0) {
    ...
    // Assuming that 'this.vector' is the <a> element
    this.vector.addEventListener('click', function() {
    ...
    // notification.textContent = 'Copied to clipboard: ' + this.innerText;
